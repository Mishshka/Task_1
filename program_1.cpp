	...
	while (ex) {
		...
		while (fl != 1) {
			...
			if ((a = str.find_first_not_of(RUS_let + num + '-')) != string::npos) {
				...
				printf("\nОшибка\nСимвольная комбинация не может оканчиваться символом '-'.\nПовторите ввод\n");
				fl = -1;
			}
			else {
...
Введение '0' в поле 'номер по списку' равносильно введению '-'.\n\n");
	...
	while (ex) {
		...
		fl = -1;
		//проверка на пустоту
		if (pas_in.password[0] == '-' and (pas_in.gr_ind[0] == '-' or pas_in.group[0] == '-'))
		{
			printf("Внимание! Вы ввели во все ключевые поля (поля 'Группа' и 'Номер по списку' символ \"-\". Пожалуйста, повторите ввод, заполнив информацией хотя бы одно из этих полей.\n\n");
		}
